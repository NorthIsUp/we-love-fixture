...
def create_wrapper(
    ...
    **attrs,
    ...
    See `@makefun.wraps`
    """
    func_name, func_sig, doc, qualname, module_name, all_attrs = _get_args_for_wrapping(wrapped, new_sig, func_name, doc, qualname, module_name, attrs)
...
def getattr_partial_aware(obj, att_name, *att_default):
    ...
    if isinstance(obj, functools.partial) and (val is None or att_name == "__dict__" and len(val) == 0):
...
def create_function(
    func_signature,  # type: Union[str, Signature]
    ...
    **attrs,
    ...
    if isinstance(func_signature, str):
        ...
        if func_name_from_str is None:
            if func_name is None:
                raise ValueError(
                    "Invalid signature for created function: `None` function name. This "
...
def get_signature_string(func_name, func_signature, evaldict):
    ...
    if no_type_hints_allowed:
        ...
        new_return_annotation = _protect_signature_symbol(func_signature.return_annotation, return_needs_protection, "RETURNHINT", evaldict)
...
def get_signature_from_string(func_sig_str, evaldict):
    ...
    if def_match is None:
        raise SyntaxError(
            'The provided function template is not valid: "%s" does not match '
    ...
    if (colon_end is None or len(colon_end) == 0) and (cmt_return_hint is None or len(cmt_return_hint) == 0):
...
def wraps(
    ...
    **attrs,
    ...
    """
    func_name, func_sig, doc, qualname, module_name, all_attrs = _get_args_for_wrapping(f, new_sig, func_name, doc, qualname, module_name, attrs)
...
def _get_args_for_wrapping(wrapped, new_sig, func_name, doc, qualname, module_name, attrs):
    ...
    if doc is None:
        ...
        if isinstance(wrapped, functools.partial) and new_sig is None and doc == functools.partial(lambda: True).__doc__:
            # this is the default generic partial doc. generate a better doc, since we know that the sig is not messed with
            doc = gen_partial_doc(
                getattr_partial_aware(wrapped.func, "__name__", None),
    ...
    **attrs,
    ...
    if func_signature is None:
        ...
        if inject_as_first_arg or not add_source or not add_impl:
            raise ValueError(
                "If `func_signature=None` no new signature will be generated so only `func_name`, "
...
def gen_partial_sig(f, preset_pos_args, preset_kwargs):
    ...
    if len(orig_sig.parameters) < len(preset_pos_args):
        raise ValueError(
            "Cannot preset %s positional args, function %s has only %s args." "" % (len(preset_pos_args), getattr(f, "__name__", f), len(orig_sig.parameters))
        )
    ...
    new_sig = Signature(parameters=tuple(new_params + new_params_with_default), return_annotation=orig_sig.return_annotation)

    if len(preset_kwargs) > 0:
        raise ValueError("Cannot preset keyword argument(s), not present in the signature of %s: %s" "" % (getattr(f, "__name__", f), preset_kwargs))
    return new_sig
        ...
        for name, cell in zip(func_code.co_freevars, func_closure):
            ...
            if name not in _evaldict:
                raise UndefinedSymbolError(
                    "Symbol %s does not seem to be defined yet. Make sure you apply " "`compile_fun` *after* all required symbols have been defined." % name
                )
            try:
                ...
                try:
                    ...
                    _evaldict[name] = compile_fun_manually(value, recurse=recurse, except_names=except_names, _evaldict=_evaldict)
